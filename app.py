...
# THE IMPACT PANEL
#
# Three columns: what pure yield-maximisation does, what AquaFair does,
# and the gap between them. The gap column is the point of the panel, so
# it is never hidden and never spun: it goes green only when AquaFair is
# actually ahead on that row, and grey — not red — when it is behind,
# because a row AquaFair loses is information, not an alarm.
#
...
def _difference(ym, aq, kind, better="high"):
    ...
    if kind == "avoided":
        ...
        # ₹185,322 less is a win, and "−₹185,322" does not look like one.
    ...
    if kind == "rupees":
        return f"{'+' if gap > 0 else '\u2212'}{_rupees(abs(gap))}", ahead
    if kind == "litres":
        return f"{'+' if gap > 0 else '\u2212'}{_litres(abs(gap))}", ahead
    return f"{'+' if gap > 0 else '\u2212'}{_kg(abs(gap))}", ahead
...
def impact_rows(sc):
    ...
    return [
        ...
        {"label": "Water actually used",
         "note": "all three policies spend the same pool — the argument is "
...
def impact_headline(sc, infeasible):
    ...
    # Losing on raw tonnage while winning on staples is the case this
...
def render_impact(out):
    """Headline, the three-column table, then the workings.
    ...
    head = (f"<tr style='border-bottom:2px solid {INK};'>"
            ...
            f"AquaFair</th>"
            f"<th style='text-align:right;padding:8px 6px;color:{GREY};"
            f"font-size:{T_SMALL};font-weight:600;'>vs current practice</th></tr>")
    ...
    for r in impact_rows(sc):
        ...
        body += (
            ...
            f"{r['fmt'](r['aq'])}</td>"
            f"<td style='text-align:right;padding:{pad} 6px;white-space:nowrap;"
            f"font-weight:700;color:{OK if ahead else GREY};{big}'>"
            f"{diff}</td></tr>")
    ...
    # column, and it reads the same way round as the Impact table above.
    metrics = [
    ...
    with st.expander("All five policies, side by side"):
        head = f"<th style='padding:6px 8px 6px 0;'></th>" + "".join(
        ...
        for name, tip, fn in metrics:
            body += (
                ...
                f"<td title='{tip}' style='padding:7px 8px 7px 0;color:{GREY};"
                f"font-size:{T_SMALL};cursor:help;'>{name}</td>" + "".join(
        ...
        st.markdown(
            ...
            f"water runs out — no method for deciding who loses. It is the "
            f"status quo this replaces: under the Lower Bhavani Project, "
            f"when storage fell short and the fifth wetting schedule was "
            f"cancelled, there was no systematic allocation and farmers "
            f"went to the RDO and then to court over who got priority.<br>"
            f"The last column counts farms that received nothing at all. "
...
def render_impact_working(out):
    ...
    block("Water actually used",
          "sum of every farm's allocation. Both columns draw on the "
          "same pool, which is why this row should read the same twice.",
    ...
    block("Compensation avoided",
          ...
          f"{_rupees(COMPENSATION_PER_ACRE_RUPEES)}, the per-acre figure "
          f"LBP farmers petitioned for. The row shows what each policy "
